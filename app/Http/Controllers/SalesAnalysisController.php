...
class SalesAnalysisController extends Controller
{
    ...
    private function buildSampleData(Carbon $startDate, Carbon $endDate): array
    {
        $days = max($startDate->diffInDays($endDate) + 1, 1);
        $avgRevenuePerDay = 4_260_000;
        $avgCostPerDay = 787_310;
        $avgInvoicePerDay = 10.37;

        $targetRevenue = $avgRevenuePerDay * $days;
        $costRate = $avgCostPerDay / $avgRevenuePerDay;

        $weights = [
            1.08, 0.9, 0.62, 0.7, 1.6, 0.8, 0.9, 0.7, 1.1, 0.95,
            0.72, 0.85, 1.4, 0.76, 0.66, 1.2, 0.82, 0.9, 0.7, 1.3,
            0.88, 0.6, 1.18, 0.92, 0.7, 0.98, 1.5, 1.0, 0.8, 1.12,
        ];
        ...
        $sumWeights = 0;
        for ($i = 0; $i < $days; $i += 1) {
            $sumWeights += $weights[$i % count($weights)];
        }
        $scale = $sumWeights > 0 ? $targetRevenue / $sumWeights : 0;

        $cursor = $startDate->copy();
        for ($i = 0; $i < $days; $i += 1) {
            $weight = $weights[$i % count($weights)];
            $revenue = (int) round($weight * $scale);
            $cost = (int) round($revenue * $costRate);
            $returns = 0;
            $profit = max($revenue - $cost - $returns, 0);
        ...
        $totalReturns = array_sum($returnSeries);
        $totalCost = array_sum($costSeries);
        $totalProfit = array_sum($profitSeries);
        $invoiceCount = (int) round($avgInvoicePerDay * $days);

        $metrics = [
            [
                'label' => 'Số hóa đơn',
                'value' => number_format($invoiceCount, 0, ',', '.'),
                'sub' => $this->formatDecimal($invoiceCount / $days) . ' đơn/ngày',
        ...
        $channelData = [
            ['label' => 'Foody', 'value' => (int) round($totalRevenue * 0.042)],
            ['label' => 'Bán trực tiếp', 'value' => (int) round($totalRevenue * 0.958)],
        ];

        $weekdayData = [
            ['label' => 'Thứ 2', 'value' => 3_240_000],
            ['label' => 'Thứ 3', 'value' => 4_010_000],
            ['label' => 'Thứ 4', 'value' => 3_190_000],
            ['label' => 'Thứ 5', 'value' => 3_770_000],
            ['label' => 'Thứ 6', 'value' => 4_650_000],
            ['label' => 'Thứ 7', 'value' => 5_060_000],
            ['label' => 'CN', 'value' => 5_990_000],
        ];

        $hourData = [
            ['label' => '0', 'value' => 156_340],
            ['label' => '1', 'value' => 14_600],
            ['label' => '2', 'value' => 0],
            ['label' => '3', 'value' => 0],
            ['label' => '4', 'value' => 0],
            ['label' => '5', 'value' => 0],
            ...
            ['label' => '8', 'value' => 0],
            ['label' => '9', 'value' => 0],
            ['label' => '10', 'value' => 13_100],
            ['label' => '11', 'value' => 64_820],
            ['label' => '12', 'value' => 84_020],
            ['label' => '13', 'value' => 0],
            ['label' => '14', 'value' => 0],
            ['label' => '15', 'value' => 115_320],
            ['label' => '16', 'value' => 428_900],
            ['label' => '17', 'value' => 418_380],
            ['label' => '18', 'value' => 617_130],
            ['label' => '19', 'value' => 692_170],
            ['label' => '20', 'value' => 655_050],
            ['label' => '21', 'value' => 596_840],
            ['label' => '22', 'value' => 402_380],
            ['label' => '23', 'value' => 0],
        ];

        $staffData = [
            ['label' => 'Nhật Thành', 'value' => 137_710_000],
            ['label' => 'Nguyễn Xuân Phúc', 'value' => 71_140_000],
            ['label' => 'Ốc Nấm Tư', 'value' => 19_770_000],
            ['label' => 'Như Quỳnh', 'value' => 19_140_000],
            ['label' => 'Nguyễn Dư', 'value' => 6_780_000],
            ['label' => 'Nguyễn Phúc Đức', 'value' => 5_180_000],
            ['label' => 'Ốc Nấm Tư', 'value' => 4_430_000],
        ];
