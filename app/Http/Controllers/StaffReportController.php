...
class StaffReportController extends Controller
{
    public function index()
    {
        ...
        return view('pos.staff-report', $reportData + [
            'dateLabel' => $dateLabel,
    ...
    {
        $baseDetails = DB::table('invoice as i')
            ->join('invoice_detail as d', 'i.id', '=', 'd.invoice_id')
            ->join('users as u', 'u.id', '=', 'i.user_id')
            ->where('i.status', 'completed')
            ->whereBetween('i.time_end', [$startTime, $endTime]);

        $staffStats = (clone $baseDetails)
            ->select(
                'u.id as staff_id',
        ...
        $totalInvoices = (clone $baseDetails)->distinct('i.id')->count('i.id');
        ...
        $prevDetails = DB::table('invoice as i')
            ->join('invoice_detail as d', 'i.id', '=', 'd.invoice_id')
            ->where('i.status', 'completed')
        ...
        $prevInvoices = (clone $prevDetails)->distinct('i.id')->count('i.id');
